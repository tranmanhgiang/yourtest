...
<table class="table table-hover">
  ...
  <?php $stt=($p-1)*$row+1; foreach($get_test as $item): ?>
    <tr>
        ...
        <?php $sta = $db->fetchAll("do_test","u_id = '".$_SESSION['u_id']."' AND t_id = '".$item['t_id']."'"); ?>
        <td>
          <div class="<?php echo ($sta != NULL ? 'btn btn-success ' : 'btn btn-light'); ?>">
            <?php echo ($sta != NULL ? 'Đã làm ' : 'Chưa'); ?>
          </div>
        </td>
        <td><?php echo ($sta != NULL ? $sta[0]['mark'] : ''); ?></td>
        <td>
          <?php if($sta != NULL): ?>
            <?php echo 'Bạn đã làm bài thi này'; ?>
          <?php else: ?>
            <a class = "btn btn-link" href="doTest.php?id=<?php echo $item['t_id']; ?>">Vào thi >></a>
          <?php endif;?>
        </td>
    </tr>
  <?php $stt++; endforeach; ?>
